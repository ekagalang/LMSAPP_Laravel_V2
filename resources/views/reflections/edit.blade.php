...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    <div class="mb-8">
        <div class="flex items-center space-x-4 mb-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-800">✏️ Edit Refleksi</h1>
                <p class="text-gray-600">Perbarui refleksi pembelajaran Anda</p>
            </div>
        </div>
    </div>

    <form action="{{ route('reflections.update', $reflection) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="title" name="title"
                       ...
                       value="{{ old('title', $reflection->title) }}" required>
                @error('title')
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-5 gap-4">
                    <label class="flex flex-col items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 transition-colors">
                        <input type="radio" name="mood" value="very_sad" class="sr-only" {{ old('mood', $reflection->mood) === 'very_sad' ? 'checked' : '' }}>
                        <span class="text-3xl mb-2">😢</span>
                        <span class="text-xs text-gray-600">Sangat Sedih</span>
                    </label>
                    <label class="flex flex-col items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 transition-colors">
                        <input type="radio" name="mood" value="sad" class="sr-only" {{ old('mood', $reflection->mood) === 'sad' ? 'checked' : '' }}>
                        <span class="text-3xl mb-2">😔</span>
                        <span class="text-xs text-gray-600">Sedih</span>
                    </label>
                    <label class="flex flex-col items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 transition-colors">
                        <input type="radio" name="mood" value="neutral" class="sr-only" {{ old('mood', $reflection->mood) === 'neutral' ? 'checked' : '' }}>
                        <span class="text-3xl mb-2">😐</span>
                        <span class="text-xs text-gray-600">Netral</span>
                    </label>
                    <label class="flex flex-col items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 transition-colors">
                        <input type="radio" name="mood" value="happy" class="sr-only" {{ old('mood', $reflection->mood) === 'happy' ? 'checked' : '' }}>
                        <span class="text-3xl mb-2">😊</span>
                        <span class="text-xs text-gray-600">Senang</span>
                    </label>
                    <label class="flex flex-col items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 transition-colors">
                        <input type="radio" name="mood" value="very_happy" class="sr-only" {{ old('mood', $reflection->mood) === 'very_happy' ? 'checked' : '' }}>
                        <span class="text-3xl mb-2">😄</span>
                ...
                <div class="space-y-3">
                    <label class="flex items-start space-x-3">
                        <input type="radio" name="visibility" value="instructors_only"
                               ...
                               {{ old('visibility', $reflection->visibility) === 'instructors_only' ? 'checked' : '' }}>
                        <div>
                    ...
                    <label class="flex items-start space-x-3">
                        <input type="radio" name="visibility" value="public"
                               class="mt-1 text-purple-600 focus:ring-purple-500"
                               {{ old('visibility', $reflection->visibility) === 'public' ? 'checked' : '' }}>
                        <div>
                    ...
                    <label class="flex items-start space-x-3">
                        <input type="radio" name="visibility" value="private"
                               class="mt-1 text-purple-600 focus:ring-purple-500"
                               {{ old('visibility', $reflection->visibility) === 'private' ? 'checked' : '' }}>
                        <div>
            ...
            <div class="mb-6">
                <label class="flex items-start space-x-3">
                    <input type="checkbox" name="requires_response" value="1"
                           class="mt-1 text-purple-600 focus:ring-purple-500"
                           {{ old('requires_response', $reflection->requires_response) ? 'checked' : '' }}>
                    <div>
...
let tags = @json(old('tags', $reflection->tags ?? []));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    moodRadios.forEach(radio => {
        ...
        if (radio.checked) {
            ...
            radio.closest('label').classList.add('border-purple-500', 'bg-purple-50');
        }
