                            ...
                            <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
                                ...
                                <?php if($content->documents && $content->documents->count()): ?>
                                    <div class="mt-6" x-data>
                                        ...
                                        <input type="hidden" name="document_order" id="document_order" value="<?php echo e($content->documents->pluck('id')->implode(',')); ?>">
                                        ...
                                        <div id="current_documents_list" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <?php $__currentLoopData = $content->documents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <div class="group relative rounded-lg overflow-hidden border bg-white p-3 flex items-center justify-between cursor-move" draggable="true" data-id="<?php echo e($doc->id); ?>">
                                                    <div class="flex items-center gap-3">
                                                        ...
                                                        <div class="min-w-0">
                                                            <p class="text-sm font-medium text-gray-900 truncate"><?php echo e($doc->original_name ?? basename($doc->file_path)); ?></p>
                                                            <a href="<?php echo e(Storage::url($doc->file_path)); ?>" target="_blank" class="text-xs text-indigo-600 hover:underline">Buka</a>
                                                        </div>
                                                    </div>
            ...
            return {
                ...
                initForm() {
                    ...
                    if (!this.content.hasOwnProperty('attendance_required')) {
                        this.content.attendance_required = <?php echo json_encode($content->attendance_required ?? false, 15, 512) ?>;
                    }
                    if (!this.content.hasOwnProperty('min_attendance_minutes')) {
                        this.content.min_attendance_minutes = <?php echo json_encode($content->min_attendance_minutes ?? null, 15, 512) ?>;
                    }
                    if (!this.content.hasOwnProperty('attendance_notes')) {
                        this.content.attendance_notes = <?php echo json_encode($content->attendance_notes ?? '', 15, 512) ?>;
                    }
